...
ggplot(data = data.frame(x = c(-3, 3)), aes(x)) +
  stat_function(fun = dnorm, n = 101, args = list(mean = 0, sd = 1), 
                ...
                size = 2) + 
  geom_vline(aes(xintercept = 0), color = "#d0d4d4", linetype = "dotted", size = 2) +
  geom_label(x=-2, y=.3, label="ohne Medienkompetenz-\nTraining", color = 'white', fill = "#d0d4d4", fontface = "bold") +
  stat_function(fun = dnorm, n = 101, args = list(mean = 1.12, sd = 1),                     # effect size = .47
                xlim = c(-3, 0),
                # geom = "area", 
                # fill = "#ff857a", 
                color = "#ff857a", 
  ...
  stat_function(fun = dnorm, n = 101, args = list(mean = 1.12, sd = 1),                     # effect size = .47
                ...
                alpha = .5,
                size = 2) + 
  geom_vline(aes(xintercept = 1.12), color = "white", linetype = "dotted", size = 1.5, alpha = .3) +    # effect size = .47
  geom_label(x=2.2, y=.3, label="mit Medienkompetenz-\nTraining", color = 'white', fill = "#ff857a", fontface = "bold") +
  geom_label(x=1, y=.15, label="87%", color = 'white', fill = "#ff857a", fontface = "bold") +
  scale_x_continuous(breaks = c(-2:+2), labels = c("- -", "-", "Lernerfolg", "+", "++")) +
  scale_y_continuous(expand = c(0,0), limits = c(0,0.41)) +
  theme_minimal() +
  theme(panel.grid = element_blank(),
...
ggsave("tudi-base/www/kom_eff.jpg",
       width = 8,
       height = 3,
       units = "mm",
       scale = 18,
